...
def cache_test_session(mock_projects_dir):
    ...
    with open(session_path, "w") as f:
        ...
        f.write(
            '{"type": "user", "uuid": "test-uuid-1", "message": {"id": "1", "content": "hi"}, "timestamp": "2024-01-01T12:00:00Z"}\n'
        )
...
def test_invalid_cursor():
    ...
    assert (
        ts is None
    )  # Should fail timestamp parsing or split check depending on implementation details
...
def test_keyset_pagination_flow(large_session_file):
    ...
    response1 = client.get(
        f"/api/sessions/{project_hash}/{session_id}/messages", params={"per_page": 10}
    )
...
async def test_tool_usage_caching(cache_test_session):
    ...
    with patch(
        "claude_code_tracer.routers.sessions.get_session_tool_usage_async",
...
async def test_metrics_caching(cache_test_session):
    ...
    # Patch the async service function where it is imported in the router (Phase 4.2 refactoring)
    with patch(
        "claude_code_tracer.routers.sessions.get_session_metrics_async",
        new_callable=AsyncMock,
