...
def test_parse_timestamp():
    ...
    assert ts == datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
...
def test_session_summary_caching(sample_session_file):
    import os
    import time
    from claude_code_tracer.services.log_parser import parse_session_summary, _cached_session_summary_impl
    ...
    summary3 = parse_session_summary(project_hash, session_id)
    info = _cached_session_summary_impl.cache_info()
    assert info.hits == 1
